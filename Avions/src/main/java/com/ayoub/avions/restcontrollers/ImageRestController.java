...
import com.ayoub.avions.service.ImageService;
...
public class ImageRestController {
    ...
    @PostMapping(value = "/uplaodImageAv/{idAv}" )
    public Image uploadMultiImages(@RequestParam("image")MultipartFile file,
                                   @PathVariable("idAv") Long idAv)
            throws IOException {
        return imageService.uplaodImageAv(file,idAv);
    }
    @RequestMapping(value = "/getImagesAv/{idAv}" ,
            method = RequestMethod.GET)
    public List<Image> getImagesAv(@PathVariable("idAv") Long idAv)
            throws IOException {
        return imageService.getImagesParAv(idAv);
    }
    @RequestMapping(value = "/uploadFS/{id}" , method = RequestMethod.POST)
    public void uploadImageFS(@RequestParam("image") MultipartFile
                                      file,@PathVariable("id") Long id) throws IOException {
        AvionDTO a = avionsService.getAvions(id);
        a.setImagePath(id+".jpg");

        Files.write(Paths.get(System.getProperty("user.home")+"/images/"+a.getImagePath())
                , file.getBytes());
        avionsService.saveAvions(a);
    ...
    public byte[] getImageFS(@PathVariable("id") Long id) throws IOException {
        ...
        AvionDTO p = avionsService.getAvions(id);
        return
